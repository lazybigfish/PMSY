...
| 多个组件 `useAuth must be used within an AuthProvider` 错误 | SystemSettings、SupplierDetailModal、ProjectOverview 等 16 个组件仍使用旧的 AuthContext | 批量修改为使用 AuthContextNew |
| REST API 404 错误 | apiClient.get 在 404 时抛出错误，导致前端显示错误 | 修改 apiClient.get，使其在 404 时返回空数组而不是抛出错误 |
| Supabase 兼容层 404 处理 | supabase.ts 的 execute 方法没有处理 404 错误 | 添加 404 错误处理，返回空数据 |
