...
#### 背景
拉取最新代码后，需要同步数据库结构以支持新功能。

#### 实施内容

1. **依赖安装**
   - 执行 `npm install` 安装 `@dnd-kit` 相关依赖
   - 解决前端启动报错 "Failed to resolve import @dnd-kit/core"

2. **数据库迁移**
   - 检查数据库表结构
   - 执行 `task_dependencies` 表创建脚本
   - 脚本内容：创建任务依赖关系表，支持 FS/SS/FF/SF 依赖类型
   - 表结构：id, task_id, depends_on_task_id, dependency_type, created_by, created_at

#### 验证结果
- ✅ npm install 执行成功
- ✅ task_dependencies 表创建成功
- ✅ 数据库表结构验证通过

#### 涉及文件
- `api-new/database/migrations/038_create_task_dependencies.sql`
- `package.json`
