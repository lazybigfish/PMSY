...
#### 问题描述
项目详情页的里程碑页面，新增里程碑阶段的任务后，控制台报错：
```
Failed to load resource: the server responded with a status of 500 (Internal Server Error)
[API] Insert error to milestone_tasks: Error: insert into "milestone_tasks" (...) 
  column "is_custom" of relation "milestone_tasks" does not exist
```

#### 问题原因
前端代码在新增任务时传入了 `is_custom: true` 字段，但数据库表 `milestone_tasks` 中缺少该字段定义。

#### 修复方案

1. **创建数据库迁移脚本** (`api-new/database/migrations/039_add_milestone_tasks_is_custom.sql`)
   - 添加 `is_custom BOOLEAN DEFAULT false` 字段
   - 为现有数据设置默认值（无 template_id 的任务设为 true）
   - 创建索引优化查询

2. **更新原始迁移文件** (`api-new/database/migrations/003_create_milestones.sql`)
   - 在表结构定义中添加 `is_custom` 字段，确保新环境部署时表结构正确

#### 涉及文件

| 文件 | 操作 |
|------|------|
| `api-new/database/migrations/039_add_milestone_tasks_is_custom.sql` | 新增迁移脚本 |
| `api-new/database/migrations/003_create_milestones.sql` | 更新表结构 |
