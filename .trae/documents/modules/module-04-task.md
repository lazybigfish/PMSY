...
#### 3.1.3 搜索与筛选栏（统计卡片与列表之间）

在统计卡片和任务列表之间增加综合搜索筛选栏，支持以下功能：

| 筛选类型      | 说明                   | 交互方式                       |
| --------- | -------------------- | -------------------------- |
| **关键词搜索** | 模糊搜索任务名称、项目名、责任人、处理人 | 文本输入框，实时搜索                 |
| **项目筛选**  | 按关联项目精确筛选            | 下拉选择框，显示所有项目列表             |
| **时间范围**  | 按任务创建时间或截止日期筛选       | 日期范围选择器                    |
| **任务状态**  | 按任务状态筛选              | 多选下拉框（未开始/进行中/已完成/已暂停/已取消） |
| **任务优先级** | 按优先级筛选               | 多选下拉框（高/中/低）               |

**搜索筛选栏布局：**

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  🔍 搜索任务、项目、责任人...    [项目 ▼ 全部]    [时间范围 ▼]    [状态 ▼]    [优先级 ▼]    [重置]  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
* **关键词搜索**：支持模糊匹配以下字段
  ...
  * 责任人姓名（creator.full\_name）

  * 处理人姓名（assignees.user.full\_name）

* **项目筛选**：单选，选择后只显示该项目的任务
...
| 用户角色 | 可见任务范围 | 说明 |
|----------|-------------|------|
| **系统管理员** | 所有任务 | 可以搜索和查看系统中的全部任务 |
| **普通用户** | 仅相关任务 | 只能搜索和查看与自己相关的任务 |

**普通用户可见的"相关任务"包括：**
1. **我创建的任务** - 用户作为责任人（creator）创建的任务
2. **我负责的任务** - 用户作为主要处理人（is_primary=true）的任务
3. **我参与的任务** - 用户作为处理人（assignee）被分配的任务

**权限控制实现：**
- 前端筛选：在 TaskList 组件中根据用户角色过滤任务列表
- 后端配合：建议配合 RLS（Row Level Security）策略确保数据安全
- 搜索范围：普通用户的搜索仅在可见任务范围内进行匹配
...
* **责任人**：单选，默认为当前登录用户，可更改，**必选**

* **处理人**：多选，默认自动添加当前登录用户作为处理人，**必选**

* 第一个选择的处理人（除创建者外）设为主要负责人（is\_primary=true）
...
| module\_id    | uuid        | 模块ID                                      |
| title         | text        | 任务标题                                      |
| description   | text        | 任务描述                                      |
| status        | text        | 状态：todo/in\_progress/paused/done/canceled |
| priority      | text        | 优先级：low/medium/high                       |
| is\_public    | boolean     | 是否公开                                      |
| due\_date     | timestamptz | 截止日期                                      |
| completed\_at | timestamptz | 完成时间                                      |
| created\_by   | uuid        | 创建人ID（即责任人）                               |
| owner\_id     | uuid        | 负责人ID（同created\_by）                       |
...
#### task\_assignees（任务处理人表）

| 字段           | 类型          | 说明        |
| ------------ | ----------- | --------- |
| id           | uuid        | 主键        |
| task\_id     | uuid        | 任务ID      |
| user\_id     | uuid        | 用户ID（处理人） |
| is\_primary  | boolean     | 是否主要负责人   |
| assigned\_at | timestamptz | 分配时间      |
...
#### task\_progress\_updates（任务进度更新表）
...
| content     | text        | 更新内容描述       |
| created\_by | uuid        | 更新人ID        |
| created\_at | timestamptz | 更新时间         |

#### task\_progress\_attachments（进度附件表）

| 字段                   | 类型          | 说明       |
| -------------------- | ----------- | -------- |
| id                   | uuid        | 主键       |
| progress\_update\_id | uuid        | 进度更新记录ID |
| file\_name           | text        | 文件名      |
| file\_url            | text        | 文件URL    |
| file\_type           | text        | 文件类型     |
| file\_size           | integer     | 文件大小（字节） |
| created\_at          | timestamptz | 上传时间     |
...
│ 管理项目任务，跟踪执行进度                                                        │
├─────────────────────────────────────────────────────────────────────────────────┤
│  📊 统计卡片区域（总任务/我负责的/我参与的/已完成/截止本周/截止本月/超期未完成）      │
├─────────────────────────────────────────────────────────────────────────────────┤
│  🔍 搜索任务、项目、责任人...    [项目 ▼ 全部]    [时间范围 ▼]    [状态 ▼]    [优先级 ▼]    [重置]  │
├─────────────────────────────────────────────────────────────────────────────────┤
...
* **新增搜索筛选栏**位于统计卡片和任务列表之间

* 搜索筛选栏支持关键词搜索、项目筛选、时间范围、状态筛选、优先级筛选
...
* **信息分组**：相关字段放在一起，减少页面空旷感

* **操作集中**：进度更新按钮放在详情页显眼位置

* **视觉层次**：使用卡片、分隔线区分不同信息块
...
│  │ 任务描述                                                 │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │ 完成系统需求分析文档，包括功能模块划分、接口设计...       │   │
│  │                                                         │   │
│  │ 这是一个多行文本区域，显示完整的任务描述内容，支持        │   │
│  │ 换行和较长的文本。描述内容放在最上方，让用户第一时间      │   │
│  │ 了解任务的具体要求和目标。                                │   │
│  │                                                         │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
...
1. **顶部（任务描述）**：完整描述内容，放在最上方让用户第一时间了解任务
2. **下方两栏布局**：

   * **左侧栏（任务信息）**：状态、优先级、责任人、处理人、项目、模块、截止日期

   * **右侧栏（进度与操作）**：进度条、更新进度按钮、最近更新摘要
3. **操作按钮**：编辑/删除放在右上角

**注意**：处理人视角下，只能修改状态和提交进度更新，其他字段均为只读。
...
| **进度更新** | **支持进度百分比、内容描述、附件上传** | **已实现** |
| **历史记录** | **显示进度更新历史，含进度条可视化** | **已实现** |
| **搜索筛选栏** | **关键词搜索、项目筛选、时间范围、状态、优先级筛选** | **已实现** |
| 高级搜索 | 支持多条件组合搜索（弹窗形式） | 已实现 |
...
| **3.2** | **2026-02-11** | **优化：任务详情页布局调整（两栏设计、进度按钮位置优化）** | **AI Assistant** |
| **3.3** | **2026-02-12** | **新增：任务列表搜索筛选栏（关键词搜索、项目/时间/状态/优先级筛选）** | **AI Assistant** |
...
* [x] 创建数据库表：`task_progress_updates`、`task_progress_attachments`
...
### 已实现：搜索筛选栏功能
- [x] **关键词搜索**：支持模糊搜索任务名称、项目名、责任人、处理人
- [x] **项目筛选下拉框**：显示所有项目列表，支持单选筛选
- [x] **时间范围筛选**：支持选择起始/结束日期，快捷选项（今天/本周/本月）
- [x] **任务状态筛选**：多选下拉框（未开始/进行中/已完成/已暂停/已取消）
- [x] **任务优先级筛选**：多选下拉框（高/中/低）
- [x] **重置按钮**：一键清空所有筛选条件
- [x] **筛选联动**：各条件之间AND关系，实时更新列表和统计卡片

### 待优化的任务详情页布局
- [ ] 调整布局为两栏设计（左侧信息 + 右侧进度与操作）
- [ ] **任务描述放在最上方独立卡片中**
- [ ] 将"更新进度"按钮从历史页移到详情页右侧栏
- [ ] 在右侧栏显示当前进度条和最近更新摘要
- [ ] 优化字段分组，减少页面空旷感
- [ ] 将编辑/删除按钮移到右上角
