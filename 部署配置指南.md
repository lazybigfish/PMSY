...
| `API_URL` | 后端API地址（必须包含端口） | `http://106.227.19.2:6969` |
| `VITE_API_URL` | 前端API地址 | `http://106.227.19.2:6969/api` |

### 部署配置文件 `.env.deploy`

首次部署时会自动生成 `.env.deploy` 文件，包含以下配置：

| 配置项 | 说明 | 示例值 |
|--------|------|--------|
| `DEPLOY_SERVER_IP` | 服务器IP地址 | `106.227.19.2` |
| `DEPLOY_SERVER_PORT` | SSH端口 | `9022` |
| `DEPLOY_SERVER_USER` | 服务器用户名 | `ubuntu` |
| `DEPLOY_REMOTE_DIR` | 远程部署目录 | `/opt/pmsy` |
...
NODE_ENV=production
PORT=3001
# ⚠️ 修改为服务器实际IP地址（必须包含端口6969）
API_URL=http://106.227.19.2:6969  # ← 修改这里

# ==========================================
# 前端配置
# ==========================================
# ⚠️ 修改为服务器实际IP地址（必须包含端口6969）
VITE_API_URL=http://106.227.19.2:6969/api  # ← 修改这里
```
...
```bash
# 1. 检查环境变量是否正确加载
ssh -p 9022 user@server 'cd /opt/pmsy && sudo docker-compose exec api env | grep -E "DB_|JWT_|MINIO_"'
